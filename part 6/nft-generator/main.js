...
const CA = "0x81f5360f7D8291C2759a907167416a12f4733ed1"
const ABI = [
  {
    "inputs": [
      {
...
document.getElementById("wallet-import-btn").addEventListener('click', async () => {
  ...
  let nftBalane = await Contract.methods.balanceOf(account.address).call()
  
  document.getElementById('eth_balance').innerText = web3.utils.fromWei(balance, 'ether');
  document.getElementById('nft_balance').innerText = nftBalane;
})

document.getElementById("nft-gen-btn").addEventListener('click', async () => {
  if (!PK) {
    alert("지갑을 복구해야 NFT를 생성할 수 있습니다.")
    return;
  }
  const file = document.getElementById("nft-file").files[0];
  const rst = await ipfs.add(file) // ipfs upload
